    ...
    <Mainlayout>
      ...
      {error && (
        <div className="p-4 m-4 bg-red-50 border border-red-200 rounded-lg">
          <p className="text-red-600 font-semibold">Error saat ambil data:</p>
          <p className="text-red-500">{error}</p>
        </div>
      )}

      {!loading && !error && originalData.length === 0 && (
        <div className="flex flex-col items-center justify-center p-12 m-4 bg-gray-50 border border-gray-200 rounded-lg">
          <svg
        ...
        <Accordion type="single" collapsible>
          {originalData.map((item, index) => (
            <AccordionItem
              ...
              value={`${item.kategori_pengadaan.toLowerCase()}-${index}`}
            >
              ...
              <AccordionContent>
                <div className="mx-auto p-6 bg-white shadow rounded-lg">
                  ...
                  <div className=" my-6 mt-6 border-t">
                    <TabelMingguan
                      idPelaksaan={item.id_pelaksanaan_pengadaan}
                    />
                  </div>
                </div>
              </AccordionContent>
